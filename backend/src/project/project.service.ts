...
export class ProjectService {
  constructor(
    ...
    @InjectModel('Share') private shareModel: Model<any>,
  ) { }
  ...
  async createProject(userId: string, payload: any) {
    ...
    if (!title) throw new BadRequestException('프로젝트 이름을 입력해주세요.');
  ...
  async getProjects(userId: string, query: any) {
    const status = ['active', 'completed', 'trash'].includes(query.status) ? query.status : 'active';
    const search = typeof query.search === 'string' ? query.search.trim() : '';
    const sort = query.sort || 'updatedAt_desc';

    const filter: any = { owner: new Types.ObjectId(userId), status };
    ...
    const countMap = {};
    designCounts.forEach(item => { countMap[item._id.toString()] = item.count; });

    let result = projects.map(p => this.toProjectDto(p, countMap[p._id.toString()] || 0));

    // 파일수 기준 정렬 수동 처리
    if (sort === 'fileCount_desc') result.sort((a, b) => b.fileCount - a.fileCount);
  ...
  async updateProjectStatus(userId: string, projectId: string, status: string) {
    ...
    const project = await this.projectModel.findOneAndUpdate(
      { _id: new Types.ObjectId(projectId), owner: new Types.ObjectId(userId) } as any,
      { status },
      { new: true }
    );
    if (!project) throw new NotFoundException('프로젝트를 찾을 수 없습니다.');

    const fileCount = await this.designModel.countDocuments({ project: project._id } as any);
  ...
  async duplicateProject(userId: string, projectId: string) {
    ...
    if (!source) throw new NotFoundException('원본 프로젝트를 찾을 수 없습니다.');
    ...
    const sourceDesigns = await this.designModel.find({ project: source._id } as any);
    if (sourceDesigns.length > 0) {
      const duplicatedDesigns = sourceDesigns.map(d => ({
